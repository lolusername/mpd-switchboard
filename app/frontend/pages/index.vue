  ...
  <div class="min-h-screen bg-[#F6F3EC]">
    ...
    <div class="ml-16 p-8">
      ...
      <div class="grid grid-cols-3 gap-4 mb-8" v-if="stats">
        ...
        <div class="bg-[#FDF1E6] rounded-2xl p-6 border border-[#FF6B35]/10 hover:shadow-lg transition-all">
          <div class="flex items-center gap-2 text-sm font-medium text-[#FA7358] mb-4">
            <svg class="w-5 h-5 text-[#FF6B35]" viewBox="0 0 24 24" fill="currentColor">
              <path d="M1.5 8.67v8.58a3 3 0 003 3h15a3 3 0 003-3V8.67l-8.928 5.493a3 3 0 01-3.144 0L1.5 8.67z" />
            </svg>
            <span class="font-semibold">Email Communications</span>
          </div>
          
          <div class="text-6xl font-bold text-[#FF6B35] mb-2 font-display tracking-tight">{{ stats.emailStats.total }}</div>
          <div class="text-sm text-[#CC4A1B]/80 font-medium">Total Emails</div>
          
          <div class="mt-6 space-y-3">
            <div class="flex justify-between items-center text-sm bg-[#FF6B35]/10 p-3 rounded-xl hover:bg-[#FF6B35]/15 transition-all">
  <span class="text-[#CC4A1B] font-medium">Internal (DC.gov)</span>
  <span class="font-semibold text-[#FF6B35]">{{ stats.emailStats.internal }}</span>
</div>
<div class="flex justify-between items-center text-sm bg-[#FF6B35]/10 p-3 rounded-xl hover:bg-[#FF6B35]/15 transition-all">
  <span class="text-[#CC4A1B] font-medium">External</span>
  <span class="font-semibold text-[#FF6B35]">{{ stats.emailStats.external }}</span>
</div>
          </div>
        </div>

        <!-- Media Communications -->
        <div class="bg-gradient-to-br from-[#2AB7CA]/5 to-[#37D4E9]/10 rounded-2xl p-6 border border-[#2AB7CA]/10 hover:shadow-lg transition-all">
          <div class="flex items-center gap-2 text-sm font-medium text-[#1A8A98] mb-4">
          ...
          <div class="text-6xl font-bold text-[#2AB7CA] mb-2 font-display tracking-tight">{{ stats.mediaStats.totalMediaEmails }}</div>
          <div class="text-sm text-[#1A8A98]/80 font-medium">Total Media Outlet Emails</div>
          
          <div class="mt-6 space-y-3">
            <div class="flex justify-between items-center text-sm bg-[#2AB7CA]/10 p-3 rounded-xl hover:bg-[#2AB7CA]/15 transition-all">
              <span class="text-[#1A8A98] font-medium">Top Media Outlet</span>
              <span class="font-semibold text-[#2AB7CA]">{{ stats.mediaStats.topMediaOutlet.domain }}</span>
            </div>
            <div class="flex justify-between items-center text-sm bg-[#2AB7CA]/10 p-3 rounded-xl hover:bg-[#2AB7CA]/15 transition-all">
              <span class="text-[#1A8A98] font-medium">Active Media Outlets</span>
              <span class="font-semibold text-[#2AB7CA]">{{ stats.mediaStats.mediaOutletCount }}</span>
            </div>
          </div>
        </div>
      </div>
